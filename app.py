...
def load_fixtures() -> pd.DataFrame:
    """
    Only a successful result is cached — st.cache_data stores return values,
    so anything returned on failure would be served to every visitor until
    the TTL expired. Raising instead leaves nothing cached, so the next
    refresh genuinely retries.
    """
    last = None
    for attempt in (1, 2, 3):
        try:
            df = _read_fixtures_once()
            break
