...
def get_engine():
    """
    No SQLAlchemy pool. Supabase's pooler already does the pooling.

    Holding a pool here on top of Supavisor meant two poolers competing for
    the same scarce slots. On port 5432 — session mode — every client
    connection reserves a real Postgres connection for its whole life, and
    with three dashboards each holding up to seven, plus the desktop sync
    scripts, the limit is reached and further connections are refused. That
    refusal arrives as OperationalError.

    NullPool opens a connection per query and closes it immediately. With
    queries cached for fifteen minutes that is a handful of connections an
    hour, and no connection is ever held long enough to go stale.

    Set SUPABASE_PORT to 6543 in Streamlit secrets — the transaction pooler.
    It multiplexes many clients onto few Postgres connections and is the
    right choice for anything serverless. psycopg2 does not use server-side
    prepared statements by default, so it works there unmodified.
    """
...
def read_sql_df(sql: str, params: dict | None = None,
    ...
    One retry on a connection-level failure: a pooler slot freeing up, or a
    dropped connection, usually resolves within a second.
    """
    try:
        try:
            ...
            print(f"[DB RETRY] {exc}")
            time.sleep(1.5)
            with ENGINE.connect() as conn:
                return pd.read_sql(text(sql), conn, params=params or {})
    except Exception as exc:
        # Full detail to the Streamlit Cloud logs (Manage app -> logs), where
        # only you can see it. Showing the exception type alone on screen
        # made this impossible to diagnose.
        print(f"[DB ERROR] {type(exc).__name__}: {exc}")
        traceback.print_exc()

        if critical:
            # Raise rather than st.stop(). The callers are @st.cache_data, and
            # that caches whatever is returned — so returning an empty frame
            # here would serve the failure to every visitor for the full TTL,
            # with refreshing unable to help. An exception caches nothing.
            raise
...
with tab_cmp:
    st.plotly_chart(
        ...
        width="stretch", config=_PLOT_CONFIG,
    )
